...
from services.export_markdown import export_project_markdown
...
class ProjectDialog(tk.Toplevel):
    def __init__(self, parent, project=None):
        ...
        self.description = tk.Text(self, height=6, bg=SURFACE_RAISED, fg=TEXT, insertbackground=TEXT, relief="flat", wrap="word")
...
class ProjectWorkspace(tk.Toplevel):
    def __init__(self, parent, storage, project):
        ...
        for row, (label, key) in enumerate((("Onde parei", "where_stopped"), ("Próximo passo", "next_step"), ("Observações", "notes")), 1):
            ttk.Label(self, text=label).grid(row=row, column=0, sticky="nw", padx=20, pady=6); text = tk.Text(self, height=3 if key != "notes" else 4, bg=SURFACE_RAISED, fg=TEXT, insertbackground=TEXT, relief="flat", wrap="word"); text.grid(row=row, column=1, sticky="ew", padx=(0, 20), pady=6); text.insert("1.0", project.get(key, "")); self.fields[key] = text
        ...
        actions = ttk.Frame(self); actions.grid(row=5, column=1, sticky="e", padx=20, pady=14); ttk.Label(actions, text="Formato:").pack(side="left", padx=4); self.text_format=ttk.Combobox(actions, state="readonly", values=("Texto simples","Markdown"), width=14); self.text_format.set("Markdown" if project.get("text_formats", {}).get("notes")=="markdown" else "Texto simples"); self.text_format.pack(side="left", padx=4); ttk.Button(actions, text="Preview", command=self.preview_text).pack(side="left", padx=4); ttk.Button(actions, text="Exportar .md", command=self.export_markdown).pack(side="left", padx=4); ttk.Button(actions, text="Prompts", command=lambda: PromptManager(self, self.storage, self.project)).pack(side="left", padx=4); ttk.Button(actions, text="Salvar", command=self.save).pack(side="left", padx=4); self.bind("<Control-s>", lambda _: self.save()); self.bind("<Escape>", lambda _: self.destroy())

    def _checklist(self, parent, column, title, values):
        box = ttk.LabelFrame(parent, text=title, padding=8); box.grid(row=0, column=column, rowspan=2, sticky="nsew", padx=(0, 8) if column == 0 else (8, 0)); box.columnconfigure(0, weight=1); box.rowconfigure(0, weight=1); listbox = tk.Listbox(box, bg=SURFACE_RAISED, fg=TEXT, selectbackground=FOCUS, relief="flat", height=10); listbox.grid(row=0, column=0, columnspan=3, sticky="nsew"); [listbox.insert("end", value) for value in values]
    ...
    def export_markdown(self):
        destination = filedialog.asksaveasfilename(parent=self, title="Exportar projeto para Markdown", defaultextension=".md", filetypes=(("Markdown", "*.md"),))
        if destination and messagebox.askyesno("Exportar", "Exportar este projeto para um arquivo Markdown local?", parent=self):
            try:
                export_project_markdown(self.project, destination)
                messagebox.showinfo("Exportar", "Projeto exportado com sucesso.", parent=self)
            except OSError as exc: messagebox.showerror("Exportar", f"Não foi possível exportar:\n{exc}", parent=self)
...
class PromptDialog(tk.Toplevel):
    def __init__(self, parent, prompt=None):
        ...
        ttk.Label(self, text=self.title(), style="Section.TLabel").grid(row=0, column=0, columnspan=2, sticky="w", padx=20, pady=(18, 12)); ttk.Label(self, text="Título").grid(row=1, column=0, sticky="w", padx=20, pady=6); self.title_entry=ttk.Entry(self); self.title_entry.grid(row=1,column=1,sticky="ew",padx=(0,20),pady=6); ttk.Label(self,text="Categoria").grid(row=2,column=0,sticky="w",padx=20,pady=6); self.category=ttk.Entry(self); self.category.grid(row=2,column=1,sticky="ew",padx=(0,20),pady=6); ttk.Label(self,text="Conteúdo").grid(row=3,column=0,sticky="nw",padx=20,pady=6); self.content=tk.Text(self,bg=SURFACE_RAISED,fg=TEXT,insertbackground=TEXT,relief="flat",wrap="word"); self.content.grid(row=3,column=1,sticky="nsew",padx=(0,20),pady=6); ttk.Label(self,text="Formato").grid(row=4,column=0,sticky="w",padx=20,pady=6); self.format=ttk.Combobox(self,state="readonly",values=("Texto simples","Markdown")); self.format.grid(row=4,column=1,sticky="ew",padx=(0,20),pady=6); self.format.set("Texto simples"); ttk.Button(self,text="Preview",command=lambda: show_markdown_preview(self,self.content.get("1.0","end").strip(),"markdown" if self.format.get()=="Markdown" else "plain")).grid(row=4,column=1,sticky="e",padx=20,pady=6); buttons=ttk.Frame(self); buttons.grid(row=5,column=0,columnspan=2,sticky="e",padx=20,pady=14); ttk.Button(buttons,text="Cancelar",command=self.destroy).pack(side="left",padx=4); ttk.Button(buttons,text="Salvar",command=self.save).pack(side="left",padx=4)
