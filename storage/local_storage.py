...
class LocalStorage:
    ...
    def create_project(self, name: str, description: str = "", status: str = "Não iniciado", text_formats: dict | None = None) -> dict:
        project = {"id": uuid4().hex, "name": name.strip(), "description": description.strip(), "status": status, "where_stopped": "", "next_step": "", "notes": "", "text_formats": text_formats or {}, "completed_checks": [], "pending_checks": [], "prompts": []}
    ...
    def save_project(self, project: dict) -> dict:
        ...
        formats = dict(project.get("text_formats", {}))
        for key in ("description", "where_stopped", "next_step", "notes"):
            formats.setdefault(key, "plain")
        project = {"id": project["id"], "name": project["name"].strip(), "description": project.get("description", "").strip(), "status": project["status"], "where_stopped": project.get("where_stopped", "").strip(), "next_step": project.get("next_step", "").strip(), "notes": project.get("notes", "").strip(), "text_formats": formats, "completed_checks": list(project.get("completed_checks", [])), "pending_checks": list(project.get("pending_checks", [])), "prompts": list(project.get("prompts", []))}
    ...
    def update_project(self, project_id: str, name: str, description: str, status: str, where_stopped: str = "", next_step: str = "", notes: str = "", completed_checks: list[str] | None = None, pending_checks: list[str] | None = None, prompts: list[dict] | None = None, text_formats: dict | None = None) -> dict:
        if not self.get_project(project_id):
            raise FileNotFoundError("Projeto não encontrado")
        current = self.get_project(project_id) or {}
        formats = dict(current.get("text_formats", {})); formats.update(text_formats or {})
        return self.save_project({"id": project_id, "name": name, "description": description, "status": status, "where_stopped": where_stopped, "next_step": next_step, "notes": notes, "text_formats": formats, "completed_checks": current.get("completed_checks", []) if completed_checks is None else completed_checks, "pending_checks": current.get("pending_checks", []) if pending_checks is None else pending_checks, "prompts": current.get("prompts", []) if prompts is None else prompts})
